...
export async function POST(req: Request) {
    try {
        ...
        // I/J열 제외, 두 번의 API 호출로 분리 기록
        const rowsPartA = rows.map((row: (string | number)[]) => row.slice(0, 8));   // A~H
        const rowsPartB = rows.map((row: (string | number)[]) => row.slice(10, 25)); // K~Y

        // 1차: A~H 추가 → 실제 시작 행 확보
        const appendResult = await sheets.spreadsheets.values.append({
            spreadsheetId: SPREADSHEET_ID,
            range: `${SHEET_NAME}!A1`,
            valueInputOption: 'RAW',
            insertDataOption: 'INSERT_ROWS',
            requestBody: { values: rowsPartA },
        });

        // 실제 기록된 시작 행 번호 파싱
        const updatedRange = appendResult.data.updates?.updatedRange || '';
        const startRowMatch = updatedRange.match(/![A-Z]+(\d+)/i);
        const startRow = startRowMatch ? parseInt(startRowMatch[1], 10) : null;

        if (!startRow) {
            throw new Error('append 시작 행 파싱 실패: ' + updatedRange);
        }

        // 2차: K~Y 동일 시작 행에 batchUpdate
        await sheets.spreadsheets.values.batchUpdate({
            spreadsheetId: SPREADSHEET_ID,
            requestBody: {
                valueInputOption: 'RAW',
                data: rowsPartB.map((row: (string | number)[], index: number) => ({
                    range: `${SHEET_NAME}!K${startRow + index}:Y${startRow + index}`,
                    values: [row],
                })),
            },
        });
