...
describe('GET and POST /api/sync-products', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it.each([
    ['GET', GET],
    ['POST', POST],
  ])('%s persists the complete validated envelope and returns generation metadata', async (_method, handler) => {
  ...
  it.each([
    ['GET', GET],
    ['POST', POST],
  ])('%s does not create a Redis client when catalog validation fails', async (_method, handler) => {
